        ...
        if current.project_id:
                ...
                if s2.task_id.kit_id:
                    self.env.cr.execute(
                        ...
                        % (s2.task_id.kit_id.id, s2.task_id.id, s2.categ_id.id, s2.product_id.id,
                           ...
                           int(s2.task_id.coordin_id9.id),
                           int(s2.task_id.coordin_id10.id),
                           int(s2.project_id.partner_id.id), int(s2.id)))
            ...
            if len(current.work_ids) > 0:
                ...
                if len(v) > 0:
                    ...
                    for kk in res_cpt:
                        if current.type == '1':
                            kk.write({'active': False, 'w_id': current.id})
                        else:
                            if current.choix == '2':
                                kk.write({'active': False, 'w_id': current.id})
                            else:
                                kk.write({'w_id': current.id})
